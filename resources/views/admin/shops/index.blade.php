                        ...
                        <tr data-entry-id="{{ $shop->id }}">
                            ...
                            <td>
                                @foreach($shop->categories as $key => $item)
                                    <span class="badge badge-info">{{ ($item->name ?? "n/a") }}</span>
                                @endforeach
                            </td>
                            <td>
                                @foreach($shop->photos as $key => $media)
                                    <a href="{{ $media->getUrl() }}" target="_blank">
                                        <img src="{{ $media->getUrl('thumb') }}" alt="" width="50px" height="50px">
                                    </a>
                                @endforeach
                            </td>
                            <td>
                            ...
                            <td>
                                @can('shop_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.shops.show', $shop->name) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('shop_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.shops.edit', $shop->name) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('shop_delete')
                                    <form action="{{ route('admin.shops.destroy', $shop->name) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
