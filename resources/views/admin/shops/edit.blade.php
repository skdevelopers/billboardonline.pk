...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.shops.update", [$shop->name]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories" id="categories" >
                    @foreach($categories as $id => $categories)
                        <option value="{{ $id }}" {{ ( old('categories') || $shop->categories->contains($id)) ? 'selected' : '' }}>{{ $categories }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="roads">{{ trans('cruds.road.fields.name') }}</label>

                <select class="form-control select2 {{ $errors->has('roads') ? 'is-invalid' : '' }}" name="roads" id="roads" >
                    @foreach($roads as $id => $road)
                        <option value="{{ $id }}" {{ ( old('roads') || $shop->roads->contains($id)) ? 'selected' : '' }}>{{ $road }}</option>
                    @endforeach
                </select>
                @if($errors->has('roads'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sizes') ? 'is-invalid' : '' }}" name="sizes" id="sizes" >
                    @foreach($sizes as $id => $size)
                        <option value="{{ $id }}" {{ ( old('sizes') || $shop->sizes->contains($id)) ? 'selected' : '' }} >{{ $size }}</option>
                    @endforeach
                </select>
                @if($errors->has('size'))
                    <div class="invalid-feedback">
...
{{--            </div>--}}
            <div class="form-group">
                ...
                <div class="form-check {{ $errors->has('featured') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="featured" id="featured" value="1" {{ $shop->featured || old('featured', 0) === 1 ? 'checked' : '' }}>
